...
ggplot(pop_sales_df %>% head(10) %>% arrange(game_num), aes(x=factor(title, levels=title), y=as.integer(game_num))) + geom_col(fill=color1) + labs(x ="Tytuł gry", y = "Liczba sprzedanych") +
  coord_flip()
```
...
ggplot(mean_keep_games %>% head(10) %>% arrange(mean_time), aes(factor(title, levels=title), mean_time)) +
  geom_col(fill=color2) + labs(x ="Tytuł gry", y = "Średnia ilość dni") + coord_flip()
```
...
ggplot(sales_df3_10, aes(x=factor(title, levels=title),y=dochod)) + geom_col(fill=color1) + 
  labs(x ="Gra", y = "Dochód") + scale_x_discrete(guide = guide_axis(angle = 45)) +
  coord_flip() 
```
...
ggplot(rentals_df3_10, aes(x=factor(title, levels=title),y=dochod)) + geom_col(fill=color4) +
  ...
  scale_x_discrete(guide = guide_axis(angle = 45)) + coord_flip()
```
