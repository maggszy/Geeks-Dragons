...
ggplot(best_empl_res, aes(x=month_date,y=as.integer(total_num), fill=as.factor(place_end))) + geom_col() +labs(x ="Miesiąc", y = "Ilość gier", fill="Miejsce") + scale_fill_manual(values=c(color1, color2, color3))+scale_x_discrete(guide = guide_axis(angle = 45))+
  geom_text(aes(label = as.integer(total_num)), size = 4,position = position_stack(vjust = 0.5), color="#E8E8E8")
```
...
ggplot(tournaments_num, aes(title, num))+geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Liczba turniejów")+scale_x_discrete(guide = guide_axis(angle = 15))
```
...
kable(top_10, caption="Top 10 graczy turniejowych z podziałem na gry.")%>%column_spec(2:6, width = "7em")%>%kable_styling(latex_options = c("HOLD_position","scale_down"))%>%row_spec(0, bold=T, background=color4)%>%column_spec(1, bold = T)
```
