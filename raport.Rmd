...
Ranking na pracownika miesiąca polega na wyłonieniu pracownika, który w danym miesiącu sumarycznie sprzedał i wynajął największą liczbę gier. W przypadku remisu pracownikiem miesiąca decydującym czynnikiem jest liczba sprzedanych gier.
...
kable(best_three, col.names=c("Miesiąc", "Najlepszy pracownik (1. miejsce)", "2. miejsce", "3. miejsce"), caption="Top 3 pracowników w poszczególnych miesiącach.", align='cccc')%>%
  column_spec(2:4, width = "10em") %>% kable_styling(latex_options = c("HOLD_position")) %>%
  row_spec(0, bold=T, background=color1)
```

```{r,  fig.cap="Ilość sprzedanych i wypożyczonych gier przez pracowników miesiąca w ostatnich 12 miesiącach."}
...
ggplot(tournaments_num, aes(title, num))+geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Liczba turniejów")+coord_flip()
```

```{r, echo=F, message=F}
best_players<-tournaments_df %>% group_by(tournament_id) %>% mutate(points=100-place+1) %>%
...
ggplot(pop_rent_df %>% head(10) %>% arrange(game_num), aes(x=factor(title, levels=title), y=as.integer(game_num))) + geom_col(fill=color4) + labs(x ="Tytuł gry", y = "Liczba wypożyczeń") + 
  coord_flip()
```
...
gamers_num <- unique(c(rent_pop_max$min_players, rent_pop_max$min_players))
```
...
ggplot(pop_sales_df %>% head(10) %>% arrange(game_num), aes(x=factor(title, levels=title), y=as.integer(game_num))) + geom_col(fill=color4) + labs(x ="Tytuł gry", y = "Liczba sprzedanych") +
  coord_flip()
```
...
gamers_num <- unique(c(sale_pop_max$min_players, sale_pop_max$min_players))
```
...
ggplot(keep_games_df%>%head(10), aes(title, day_num, fill=factor(ifelse(rental_id %in%rental_id_max$rental_id,"Maksimum","Pozostałe")))) +
  geom_col(position = position_dodge2())+scale_fill_manual(name = "area", values=c(color3,color1)) +
  theme(legend.position = "none") + labs(x ="Tytuł gry", y = "Ilość dni") + coord_flip()
```

```{r, echo=F}
keep_games_table<- keep_games_df %>% head(10) %>% 
  select(title, customer_name, rental_date, day_num) %>% 
  mutate(rental_date=format(rental_date, "%d.%m.%Y"))

kable(keep_games_table, col.names=c("Tytuł gry","Klient","Data wypożyczenia","Długość wypożyczenia"), caption="Najdłużej przetrzymywane gry w historii sklepu wraz z datami wypożyczenia.", align='llcc') %>% 
  column_spec(1:2, width = "11em") %>% column_spec(3:4, width = "7em") %>% 
...
Najdłuższy średni czas przetrzymywania to **`r round(mean_max, 3)`** uzyskany przez grę **`r mean_max_game$title`**.
...
## Gry przynoszące największy dochód ze sprzedaży

<!-- ```{r echo=F} -->
<!-- # 10 sprzadawanych gier przynoszącoych największy dochoód -->
<!-- # sales_df3_10 <- sales_df3 %>% select(-game_id) %>% slice_head(n=10)  -->
<!-- # sales_df3_10$index <- seq_len(nrow(sales_df3_10)) %>% rev() -->


<!-- #raczej niepotrzebne, jak mamy wykres -->
<!-- # kable(sales_df3_10[,1:2], col.names=c("Tytuł gry","Dochód"), caption="Gry przynoszące największy dochód z sprzdaży.", align='lc') %>% column_spec(1, width = "18em") %>% column_spec(2, width = "9em") %>% -->
<!-- #   kable_styling(latex_options = c("HOLD_position","scale_down"),position = "center") %>% row_spec(0, bold=T, background=color1)  -->
<!-- ``` -->
...
ggplot(sales_df3_10, aes(x=factor(title, levels=title),y=dochod)) + geom_col(fill=color1) + 
  labs(x ="Gra", y = "Dochód") + scale_x_discrete(guide = guide_axis(angle = 30)) +
  coord_flip() 
```

<!-- komentarz, które gry przyniosły największy dochód -->

## Gry przynoszące największy dochód z wypożyczeń

<!-- ```{r} -->
<!-- # 10 wypożyczonych gier przynoszącoych największy dochoód -->
<!-- # rentals_df3_10 <- rentals_df3 %>% select(-game_id) %>% slice_head(n=10) -->
<!-- # rentals_df3_10$index <- seq_len(nrow(rentals_df3_10))  -->

<!-- #raczej niepotrzebne, jak mamy wykres -->
<!-- # kable(rentals_df3_10[,1:2], col.names=c("Tytuł gry","Dochód"), caption="Gry przynoszące największy dochód z wypożyczeń.",  align='lc') %>% column_spec(1, width = "18em") %>% column_spec(2, width = "9em") %>% -->
<!-- #   kable_styling(latex_options = c("HOLD_position","scale_down"),position = "center") %>%  -->
<!-- #   row_spec(0, bold=T, background=color1) -->
<!-- ``` -->
...
```{r}
sales_income <- 'SELECT DATE_FORMAT(date, "%Y-%m") as ym, EXTRACT(YEAR FROM Finances.date) AS year, EXTRACT(MONTH FROM Finances.date) AS month, SUM(Finances.value) AS dochod
FROM Sales
INNER JOIN Finances USING(payment_id)
INNER JOIN Inventory USING(inventory_id)
GROUP BY year, month;'

rentals_income <- 'SELECT DATE_FORMAT(date, "%Y-%m") as ym, EXTRACT(YEAR FROM Finances.date) AS year, EXTRACT(MONTH FROM Finances.date) AS month, SUM(Finances.value) AS dochod
FROM Rentals
INNER JOIN Finances USING(payment_id)
INNER JOIN Inventory USING(inventory_id)
GROUP BY year, month;'
...
kable(income[,c(1,4:7)], col.names=c("Data","Sprzadaż","Wypożyczenia","Turnieje","Przychód"), caption="Przychódy sklepu w rozbiciu na typ przychodu w poszczególnych miesiącach.", align='ccccc')  %>%
  column_spec(1:5, width = "9em") %>% #column_spec(2, width = "6em") %>%
  kable_styling(latex_options = c("HOLD_position","scale_down")) %>% row_spec(0, bold=T, background=color1)
```

```{r fig.cap = "Przychódy w poszczególnych miesiącach z podziałem na typ przychodu."}
...
ggplot(income_long, aes(x=ym,y=as.integer(value), fill=as.factor(type))) + geom_col() + 
  ...
  # geom_text(aes(label = as.integer(value)), size = 3,position = position_stack(vjust = 0.5), color="#E8E8E8")
```

## Dochód sklepu w poszczególnych miesiącach

```{r}
in_out <- 'SELECT DATE_FORMAT(date, "%Y-%m") as ym, EXTRACT(YEAR FROM date) AS year, EXTRACT(MONTH FROM date) AS month, in_out, SUM(value) AS income
FROM Finances
...
<!-- # Dochód sklepu w poszczególnych miesiącach - ogólny -->
```{r}
...
kable(month_in_out[,c(1,3:5)], col.names=c("Data","Przychód","Wydatki","Dochód"), caption="Całkowity dochód sklepu w poszczególnych miesiącach.",  align='cccc')  %>% column_spec(1:4, width = "9em") %>% 
  kable_styling(latex_options = c("HOLD_position","scale_down")) %>% row_spec(0, bold=T, background=color1)
```
...
```{r}
# wybrać tylko te dane, gdzie rok jest obecny_rok - 1
# year <- in_out_df$year %>% unique()
...
Całkowity dochód firmy od **`r start_end[1]`** do **`r start_end[2]`** wyniósł: **`r total_income`** [zł]. Daje nam to średnio: **`r round(mean_income,2)`** [zł] dochodu na miesiąc.
