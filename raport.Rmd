...
knitr::opts_chunk$set(echo = TRUE, warning = F, fig.pos="H",out.width = "75%", fig.align = 'center')
library(RMariaDB)
library(ggplot2)
library(dplyr)
library(dbplyr) 
...
kable(best_three, col.names=c("Miesiąc", "Najlepszy pracownik (1. miejsce)", "2. miejsce", "3. miejsce"), caption="Top 3 pracowników w poszczególnych miesiącach.")%>%
  column_spec(2:4, width = "10em")%>%kable_styling(latex_options = c("HOLD_position"))%>%row_spec(0, bold=T, background=color4)
```
...
```{r,echo=F, fig.cap="Liczba rozegranych turniejów w zależności od gry.", out.width = "65%"}
...
kable(top_10, caption="Top 10 graczy turniejowych z podziałem na gry.")%>%column_spec(2:(tournament_unique_num+1), width = "7em")%>%kable_styling(latex_options = c("HOLD_position","scale_down"))%>%row_spec(0, bold=T, background=color4)%>%column_spec(1, bold = T,background=color4)
```
...
```{r, echo=F}
ggplot(pop_rent_df %>% head(10) %>% arrange(game_num), aes(x=factor(title, levels=title), y=as.integer(game_num)))+geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Liczba wypożyczeń")+coord_flip()
```
...
```{r, echo=F}
ggplot(pop_sales_df %>% head(10) %>% arrange(game_num), aes(x=factor(title, levels=title), y=as.integer(game_num)))+geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Liczba sprzedanych")+coord_flip()
```
...
ggplot(mean_keep_games%>%head(10)%>% arrange(mean_time), aes(factor(title, levels=title), mean_time)) + geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Średnia ilość dni")+coord_flip()
```
...
sales_df3_10 <- sales_df3 %>% select(-game_id) %>% #mutate(dochod=dochod*0.001)
  slice_head(n=10)
sales_df3_10$index <- seq_len(nrow(sales_df3_10))

#poprawić wygląd, bo są za duże
kable(sales_df3_10[,1:2], col.names=c("Tytuł gry","Dochód"), caption="Gry przynoszące największy dochód z sprzdaży.") %>% column_spec(1:2, width = "11em") %>%
  kable_styling(latex_options = c("HOLD_position","scale_down"),position = "center") %>% row_spec(0, bold=T, background=color4)
```


```{r echo=F}
#dopracować ten wykres tak, żeby ładnie wyglądał - łącznie z kolorkami
ggplot(sales_df3_10, aes(x=index,y=as.integer(dochod), fill=title)) + geom_col() +labs(x ="Gra", y = "Dochód", fill="Gra") + scale_fill_discrete(name="") + scale_x_discrete(guide = guide_axis(angle = 45)) +
...
rentals_df3_10 <- rentals_df3 %>% select(-game_id) %>% #mutate(dochod=dochod*0.001)
  slice_head(n=10)
rentals_df3_10$index <- seq_len(nrow(rentals_df3_10))

kable(rentals_df3_10[,1:2], col.names=c("Tytuł gry","Dochód"), caption="Gry przynoszące największy dochód z wypożyczeń.") %>% column_spec(1:2, width = "11em") %>%
  kable_styling(latex_options = c("HOLD_position","scale_down"),position = "center") %>% row_spec(0, bold=T, background=color4)
```
...
ggplot(rentals_df3_10, aes(x=index,y=as.integer(dochod), fill=title)) + geom_col() +labs(x ="Gra", y = "Dochód", fill="Gra") + scale_fill_discrete(name="") + scale_x_discrete(guide = guide_axis(angle = 45)) +
 scale_fill_brewer(palette = "RdGy") 
```
