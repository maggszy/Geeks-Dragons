...
knitr::opts_chunk$set(echo = TRUE, fig.pos="H",out.width = "75%", fig.align = 'center')
library(RMariaDB)
library(ggplot2)
library(dplyr)
library(dbplyr)
...
kable(top_10, caption="Top 10 graczy turniejowych z podziałem na gry.")%>%column_spec(2:6, width = "7em")%>%kable_styling(latex_options = c("HOLD_position","scale_down"))%>%row_spec(0, bold=T, background=color4)%>%column_spec(1, bold = T,background=color4)
```
...
ggplot(mean_keep_games%>%head(10), aes(factor(title, levels=title), mean_time)) + geom_col(fill=color1)+labs(x ="Nazwa gry", y = "Średnia ilość dni")+coord_flip()
```
