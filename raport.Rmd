...
```{r, echo=F}
ggplot(pop_rent_df %>% head(10), aes(x=factor(title, levels=title), y=as.integer(game_num)))+geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Liczba wypożyczeń")+coord_flip()
```
...
```{r, echo=F}
ggplot(pop_sales_df %>% head(10), aes(x=factor(title, levels=title), y=as.integer(game_num)))+geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Liczba sprzedanych")+coord_flip()
```
...
ggplot(mean_keep_games%>%head(10), aes(factor(title, levels=title), mean_time)) + geom_col(fill=color1)+labs(x ="Tytuł gry", y = "Średnia ilość dni")+coord_flip()
```
