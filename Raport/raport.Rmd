...
W przypadku remusów gry uszeregowane są w kolejności alfabetycznej.
...
ggplot(pop_rent_df %>% head(10) %>% arrange(game_num, desc(title)), aes(x=factor(title, levels=title), y=as.integer(game_num))) + geom_col(fill=color4) + labs(x ="Tytuł gry", y = "Liczba wypożyczeń") + 
  coord_flip()
```
...
ggplot(keep_games_df, aes(day_num)) +
  geom_bar(fill=color3)+
  theme(legend.position = "none") + labs(x ="Długość wypożyczenia", y = "Ilość wypożyczeń")
```
Gra najdłużej była przetrzymywana przez **`r day_num_max`** dni. Gry, które były przetrzymywane przez tyle dni zostały przedstawione w tabeli 3.
...
kable(keep_games_table, col.names=c("Tytuł gry","Klient","Data wypożyczenia"), caption="Najdłużej przetrzymywane gry w historii sklepu wraz z datami wypożyczenia.", align='llcc')%>%kable_styling(latex_options = c("HOLD_position","scale_down"),position = "center") %>% 
  row_spec(0, bold=T, background=color1)
```
